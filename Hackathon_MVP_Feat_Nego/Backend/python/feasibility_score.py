...
# Define LLM components at the module level for reusability
# Consider moving the API key to an environment variable for security
prompt_template_global = PromptTemplate.from_template("""
...
parser_global = JsonOutputParser()
llm_global = ChatGoogleGenerativeAI(
    model="gemini-1.5-flash",
    google_api_key="AIzaSyCp8H9Ihvgujw76b56eIVQOAK8Jr92YBpo", # Replace with os.getenv("GOOGLE_API_KEY")
    temperature=0.3,
)
...
def assess_feasibility(input_features_list: list) -> list:
    ...
    for feature_data in input_features_list:
        try:
            prompt_filled = prompt_template_global.format(feature=json.dumps(feature_data))
            output = llm_global.invoke(prompt_filled)
            parsed = parser_global.invoke(output)
